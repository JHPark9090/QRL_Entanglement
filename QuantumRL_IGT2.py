...
filename2= 'lo_100.csv'
loss_data = pd.read_csv(filename2)
filename3= 'choice_100.csv'
...
choice_data.iloc[:, 1:101] = choice_data.iloc[:, 1:101]-1

# Number of choices (A, B, C, D)
num_choices = 4

# Extract subjects and trial columns
subjects = choice_data['subject'].values
trial_columns = [col for col in choice_data.columns if col != 'subject']
...
for i, subject in enumerate(subjects):
    for j, trial_column in enumerate(trial_columns):
        choice = choice_data.loc[choice_data['subject'] == subject, trial_column].values[0]
...
class IGTEnv(gym.Env):
    def __init__(self, choice_data, acc_rewards, seed):
      # choices_data: Iowa Gambling Task Choices data: pandas dataframe
      # acc_rewards: Iowa Gambling Task Accumulated Net Rewards data: numpy array
        ...
        self.subjects = np.unique(choice_data['subject'])
        self.trials = [f'trial_{i}' for i in range(1, len(choice_data.columns))]
    ...
    def reset(self, seed=None):
        ...
        self.current_trial_idx = 0
        self.current_subject_idx = random.randint(0, len(np.unique(choice_data['subject']))-1)      
#        self.total_return = 0
...
env = IGTEnv(choice_data, acc_rewards, seed=2023)

train_envs = DummyVectorEnv([lambda: IGTEnv(choice_data, acc_rewards, seed=2023) for _ in range(args.training_num)])
test_envs = DummyVectorEnv([lambda: IGTEnv(choice_data, acc_rewards, seed=2023) for _ in range(args.test_num)])
