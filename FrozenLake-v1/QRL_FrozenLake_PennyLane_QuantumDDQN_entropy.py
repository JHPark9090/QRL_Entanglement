...
class QuantumDQN(nn.Module):
    def __init__(self, n_qubits, n_actions, n_layers, w_input, w_output, data_reupload):
        ...
        self.q_layers = get_model(n_qubits=self.n_qubits,
                                  ...
                                  return_val=True)  # Set return_entropy=False to get the model
        self.entropy = get_model(n_qubits=self.n_qubits,
                                  n_layers=n_layers,
                                  data_reupload=data_reupload,
                                  return_val=False)  # Set return_entropy=True to get the entropy
        if w_input:
...
writer = SummaryWriter(f'log_{args.log_num}/PennyLane_{args.task}_Quantum_DDQN')
...
path = f'/scratch/connectome/justin/log_{args.log_num}/PennyLane_{args.task}_Quantum_DDQN_{args.log_num}.pth'
torch.save(policy.state_dict(), path)

policy.load_state_dict(torch.load(path))
policy.eval()
result = test_collector.collect(n_episode=100, render=False)
print("Quantum Double DQN Final reward: {}, length: {}".format(result["rews"].mean(), result["lens"].mean()))


path = f'/scratch/connectome/justin/log_{args.log_num}/PennyLane_{args.task}_Quantum_DDQN_{args.log_num}.hdf5'
...
buffer_res.to_csv(f'/scratch/connectome/justin/log_{args.log_num}/PennyLane_{args.task}_Quantum_DDQN_{args.log_num}.csv')
...
df = df.transpose()
df.to_csv(f'/scratch/connectome/justin/log_{args.log_num}/PennyLane_{args.task}_Quantum_DDQN_entropy_{args.log_num}.csv')
