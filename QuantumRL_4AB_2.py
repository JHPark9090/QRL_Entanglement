...
def get_args():
    ...
    parser.add_argument("--task", type=str, default='4AB')
...
filename= 'DataAllSubjectsRewards_imputed.csv'
bandit_data = pd.read_csv(filename)
bandit_data['choice'] = bandit_data['choice'].astype('category')
bandit_data['trial'] = bandit_data.groupby('id').cumcount() + 1

# Iterate through unique ids and trials and extract choice and reward values
unique_ids = bandit_data['id'].unique()
unique_trials = bandit_data['trial'].unique()

# Create an empty 4D numpy array to store the state representations
state_array = np.zeros((len(unique_ids), len(unique_trials), 4))

for id_val in unique_ids:
    for trial_val in unique_trials:
        subset = bandit_data[(bandit_data['id'] == id_val) & (bandit_data['trial'] == trial_val)]
        choice = subset['choice'].values[0]
        reward = subset['reward'].values[0]
        state_array[id_val-1, trial_val-1, choice-1] = reward

# Calculate accumulated rewards for each choice across trials for each subject
...
class BanditEnv(gym.Env):
    def __init__(self, dataset, acc_rewards, seed):
        # Load the dataset
        self.df = dataset
        self.rewards_data = acc_rewards

        # Define the action and observation space
        # Observation space: 4-dimensional vector
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(4,), dtype=np.float32)
#         self.observation_space = spaces.Discrete(4)
        # 4-armed bandit, 4 possible actions
    ...
    def reset(self, **kwargs):
        ...
        self.current_participant = 0
        self.total_return = 0
        return self._get_state(), {}  # Replace state with any appropriate initial observation value

    def _get_state(self):
        state = self.rewards_data[self.current_participant][self.current_trial]
        return state
    
    def step(self, action):
        # Execute the chosen action and observe the new state and reward
        participant_data = self.df[
            (self.df["id"] == self.current_participant) &
            (self.df["choice"] == action)
        ]

        # Check if participant_data is empty
        if participant_data.empty:
            # Handle the case when the participant_data is empty
            reward = 0  # Replace 0 with any appropriate default value for rewards
        else:
            reward = participant_data["reward"].values[0]

        self.total_return += reward
        # Move to the next trial
        self.current_trial += 1
        # Check if the episode is over
        done = self.current_trial >= self.df["trial"].nunique()-1
        # Update the participant index for the next trial
        if not done:
            self.current_participant = self.df["id"].unique()[self.current_trial]
    ...
    def render(self, mode='human'):
        ...
        print(f"Participant: {self.current_participant}, Trial: {self.current_trial}, Total Return: {self.total_return}")

    def seed(self, seed):
        np.random.seed(seed)
        
env = BanditEnv(bandit_data, acc_rewards, seed=2023)

train_envs = DummyVectorEnv([lambda: BanditEnv(bandit_data, acc_rewards, seed=2023) for _ in range(args.training_num)])
test_envs = DummyVectorEnv([lambda: BanditEnv(bandit_data, acc_rewards, seed=2023) for _ in range(args.test_num)])

# Use your defined network
state_shape = env.observation_space.shape[0] or env.observation_space.n  # equivalent to 4 for 4-Armed Bandit Task
action_shape = env.action_space.n  # equivalent to 4 for 4-Armed Bandit Task
...
buffer_df = pd.DataFrame(buffer_obs, columns=['Arm_A', 'Arm_B', 'Arm_C', 'Arm_D'])
buffer_df2 = pd.DataFrame(buffer_obs_next, columns=['Arm_A2', 'Arm_B2', 'Arm_C2', 'Arm_D2'])
